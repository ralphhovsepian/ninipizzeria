...
var mobilenavUl = document.getElementById('mobilenav');
var shoppingcart = document.querySelectorAll('#shoppingcart');
var cart = document.querySelector('.cart');
var pepperoniPizza = document.querySelector('#pepperoni');
var hawaiianPizza = document.querySelector('#hawaiian');
var bbqPizza = document.querySelector('#bbq');
var margheritaPizza = document.querySelector('#margherita');
var summary = document.getElementById('summary');
var totalparag = document.getElementById('totalparag');
var pizzaBtns = [pepperoniPizza, hawaiianPizza, bbqPizza, margheritaPizza];
var pepperonisummary = document.getElementById('pepperonisummary');
var hawaiiansummary = document.getElementById('hawaiiansummary');
var bbqsummary = document.getElementById('bbqsummary');
var margheritasummary = document.getElementById('margheritasummary');
var price = 0;
var pizza = {
  pepperoni: {
    name: 'pepperoni',
...
var addtocart = function addtocart() {
  pizzaBtns.forEach(function (pizzaBtn) {
...
var getPrice = function getPrice(pizzaType) {
  return pizza[pizzaType].total = pizza[pizzaType].price * pizza[pizzaType].quantity;
}; //show items in the shopping cart + total


var showItems = function showItems() {
  price = 0;
  Object.entries(pizza).forEach(function (type) {
    if (type[1].quantity > 0) {
      price += getPrice(type[1].name); //summary.innerHTML = `<br>${type[1].quantity} ${type[1].name} pizza`;
    }
  });

  if (pizza.pepperoni.quantity > 0) {
    pepperonisummary.innerHTML = "<br>".concat(pizza.pepperoni.quantity, " ").concat(pizza.pepperoni.name, " pizza");
  }

  if (pizza.bbq.quantity > 0) {
    bbqsummary.innerHTML = "<br>".concat(pizza.bbq.quantity, " ").concat(pizza.bbq.name, " pizza");
  }

  if (pizza.hawaiian.quantity > 0) {
    hawaiiansummary.innerHTML = "<br>".concat(pizza.hawaiian.quantity, " ").concat(pizza.hawaiian.name, " pizza");
  }

  if (pizza.margherita.quantity > 0) {
    margheritasummary.innerHTML = "<br>".concat(pizza.margherita.quantity, " ").concat(pizza.margherita.name, " pizza");
  }

  totalparag.innerHTML = "Total: $".concat(price);
}; //shopping cart show, hide


shoppingcart.forEach(function (shopping) {
